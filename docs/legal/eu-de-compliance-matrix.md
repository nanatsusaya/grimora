...
| **DSGVO Art. 33/34** (data-breach notification) | In force | **Yes at go-live** — a personal-data breach must be notified to the supervisory authority within **72 h** (Art. 33) and to affected persons where high-risk (Art. 34); this requires a documented **incident / breach-response plan**. **Premature to write today** (the dev Supabase holds only fake smoke data) → a **go-live gate**, not a current task. | ADR 0015 (compliance ops) + a future `docs/ops/` incident-response runbook (alongside the rotation/restore runbooks, ADR 0014) |
