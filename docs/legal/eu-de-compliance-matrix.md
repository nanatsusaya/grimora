...
| **BFSG** (Barrierefreiheitsstärkungsgesetz / EAA) | In force since **28 Jun 2025**, no transition period for web/apps | **Likely exempt today** — micro-enterprise-providing-services exemption (≤ 2M€ turnover/balance sheet) probably applies to a solo-owner project; **becomes mandatory once that threshold is crossed** (e.g. paid tiers scale up) | ADR 0007 (already commits to WCAG 2.2 AA voluntarily); ADR 0016 (accessibility ADR, formalizes) |
...
| **Jugendschutz (JMStV/JuSchG)** | In force | **Depends on product decisions not yet made** — user-generated campaign content/images could be developmentally-sensitive; needs age-rating/labelling and possibly technical protections (age gate) if content moderation doesn't bound this | Open question for ADR 0010 (threat model) / product ToS — no decision yet |
...
- Several rows (Fernabsatzrecht, JMStV) currently have **no owning ADR** — this is a known gap for
  ADR 0010/0015 to close, not an oversight to silently carry forward.
- "Applies to Grimora?" assessments assume the **current** project shape (solo owner + AI agents, no
  employees, pre-revenue). Several exemptions (BFSG micro-enterprise, NIS2 size threshold) are
  **scale-dependent** — re-check this matrix whenever the project crosses a size/revenue milestone or
  ships a new distribution channel (mobile/desktop apps trigger CRA product scope).
