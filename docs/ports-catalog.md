...
| `SyncPort` | Insert-only replication + domain rebase against the cloud | ADR 0005 §3/§4, ADR 0011 §7, ADR 0024 | **Real (push + pull)** — the `SyncPort` interface (`push`/`pull`) + in-memory fake (`createInMemorySyncPort`) held to `syncPortContract` (#107 PR A); the **server** side is real (`apps/api` Postgres event store + `/sync/push`+`/sync/pull` with JWKS actor-binding, #107 slice 1/2, live-verified against Supabase); the **client** side is real for both halves — `@grimora/offline-sync` `createHttpSyncPort` + `createSyncService` (`pushPending` **and** `pullPending`, the latter applying pulled events via the event store's `replicate`), wired at the `apps/web` composition root (login-triggered sync + "Sync now"; the view re-projects after a pull), live-verified end-to-end (`sync-push-smoke.ts` + the two-device `sync-pull-smoke.ts`). A read-model **character index** + picker (slice 3c) makes a newly-pulled cross-device character browsable/openable in the UI (Playwright-verified). The domain **rebase** on `conflict` + cross-device co-editing are deferred (parked, #176). | Custom sync engine over Supabase Postgres (`apps/api` + `packages/offline-sync`) | #107 — PR A ✅; slice 1 ✅; slice 2 ✅; slice 3a push ✅; slice 3b pull/apply ✅; slice 3c picker ✅; co-editing/rebase → #176 |
