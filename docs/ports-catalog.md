...
| `SyncPort` | Insert-only replication + domain rebase against the cloud | ADR 0005 §3/§4, ADR 0011 §7, ADR 0024 | **Real (push); pull transport real, local-apply next** — the `SyncPort` interface (`push`/`pull`) + in-memory fake (`createInMemorySyncPort`) held to `syncPortContract` (#107 PR A); the **server** side is real (`apps/api` Postgres event store + `/sync/push`+`/sync/pull` with JWKS actor-binding, #107 slice 1/2, live-verified against Supabase); the **client** side is real for **push** — `@grimora/offline-sync` `createHttpSyncPort` (HTTP `SyncPort`) + `createSyncService` (push orchestration), wired at the `apps/web` composition root (push-on-login + "Sync now"), live-verified end-to-end (`apps/web/scripts/sync-push-smoke.ts`). `pull` transport exists; **local idempotent apply + projection rebuild (cross-device view) is slice 3b**, and the domain **rebase** on `conflict` + cross-device co-editing are deferred (parked, #176). | Custom sync engine over Supabase Postgres (`apps/api` + `packages/offline-sync`) | #107 — PR A (contract) ✅; slice 1 migration ✅; slice 2 endpoints ✅; slice 3a client push ✅; slice 3b pull/view next; co-editing/rebase → #176 |
