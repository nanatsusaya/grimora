...
| `AuthPort` | Authentication: sign-in, session, session-change subscription (token issuance/refresh/storage is the adapter/composition-root concern, ADR 0012 §5) | ADR 0009 §3, ADR 0012 §5/§13 | **Skeleton fake** — the `AuthPort` interface (`signIn`/`signOut`/`getSession`/`onSessionChange` + `AuthSession`/`AuthCredentials`) now exists in `ports.ts` with an in-memory fake (`createInMemoryAuthPort`) unit-tested; no real adapter yet (#120 E1) | Supabase Auth via the `apps/api` auth proxy (web, ADR 0012 §5) / self-hosted GoTrue (ADR 0009 §3) | #120 — E1 (interface) ✅; Supabase adapter + `apps/api` proxy (E2), login UI + §5 token storage (E3), §13 first-bind (E4) |
