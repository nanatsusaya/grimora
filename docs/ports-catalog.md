...
| --- | --- | --- | --- | --- | --- |
| `EventStorePort` | Append-only per-aggregate event log, optimistic concurrency via `expectedVersion` | ADR 0004 §4, ADR 0005 §1 | Skeleton fake (`createInMemoryEventStore`) | SQLite (native) / OPFS (web) local; Postgres (Supabase) cloud via sync | *Local event-store adapter* ticket (to be opened) |
