...
| `ReadModelStorePort` | Denormalized, checkpointed read models the UI queries — never the event store | ADR 0004 §5, ADR 0005 §1 | **Real (native)** — `@grimora/cqrs-read` `createSqliteReadModelStore` (`bun:sqlite`, durable, rebuild-from-0 via `clear()`), held to the shared `readModelStoreContract`; the `characterSheet` projection runs over it + the real event store end-to-end; in-memory fake still used for pure tests. **Web (OPFS) driver deferred to #105.** | SQLite native ✅ / OPFS (web) → #105; cloud projections via sync → #107 | #104 ✅ (native); OPFS sub-task under #105 |
